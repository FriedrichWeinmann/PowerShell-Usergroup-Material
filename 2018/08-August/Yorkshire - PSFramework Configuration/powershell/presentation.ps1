...
Register-PSFConfigValidation -Name "ipaddress" -ScriptBlock {
	Param (
		$Value
	)
	...
	if (-not ($Value -as [IPAddress]))
	{
		$Result.Message = "Not an IP Address!: $Value"
		$Result.Success = $False
		return $Result
	}
	
	$Result.Value = $Value -as [IPAddress]
	
	return $Result
}

Set-PSFConfig -Module Foo -Name TargetAddress -Value ([IPAddress]127.0.0.1) -Validation ipaddress
Set-PSFConfig -FullName 'Foo.TargetAddress' -Value "foo"
Get-PSFConfig -FullName 'Foo.TargetAddress'
Set-PSFConfig -FullName 'Foo.TargetAddress' -Value "::1"
Get-PSFConfig -FullName 'Foo.TargetAddress'
